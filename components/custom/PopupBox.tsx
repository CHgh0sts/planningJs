...
interface PopupBoxProps {
  event?: any;
  heureAddEvent?: string;
}

export const PopupBox: React.FC<PopupBoxProps> = ({ event = null, heureAddEvent }) => {
  ...
  useEffect(() => {
    ...
    setAvailableUsers(usersList.filter((user: { id: any; }) => !selectedUsers.some(selected => selected.id === user.id)));
  ...
  const adjustEndTime = (newStartTime: string, currentEndTime: string) => {
